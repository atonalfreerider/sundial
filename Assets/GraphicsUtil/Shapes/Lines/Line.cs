...
namespace Assets.GraphicsUtil.Shapes.Lines
{
    public class Line : Polygon
    {
        public void DrawLine(Vector3[] points, float LW, bool closed, int n)
        {
            ...
            {
                prevPt = GetPrev(ii, points);
                currPt = points[ii];
                nextPt = GetNext(ii, points);

                // determine offset based on angle at joint;
                vec1 = currPt - prevPt;
                vec2 = nextPt - currPt;
                alpha = Vector3.Angle(vec1, vec2) * .5f;
                x = LW * .5f * Mathf.Atan(alpha * Mathf.PI / 180f);
                d = Vector3.Distance(prevPt, currPt);
                backPt = Vector3.Lerp(prevPt, currPt, 1f - x / d);

                // add Line;    
                LineArray(fwdPt, backPt, LW, n).CopyTo(skinList, (ii - startInd) * 4);

                // set forward for next iteration;
                fwdPt = Vector3.Lerp(currPt, nextPt, x / Vector3.Distance(currPt, nextPt));

                // Tri 1;
                insInd = new[] {0 + prevIndices, 2 + prevIndices, 3 + prevIndices};
                ...
                {
                    if (alpha <= 0)
                        insInd = new[] {0 + prevIndices, -2 + prevIndices, 1 + prevIndices};
                    else
                        insInd = new[] {0 + prevIndices, 1 + prevIndices, -1 + prevIndices};
        ...
        static Vector3[] LineArray(Vector3 ptA, Vector3 ptB, float LW, int n)
        {
            ...
            {
                addPt = Vector3.Normalize(Quaternion.AngleAxis(ii * alpha - 90f, cross) * normal);
                addPt *= LW * .5f;
                addPt += ptA;
                points[ii] = addPt;
            }
        ...
        {
            return ind == 0 ? passV[passV.Length - 1] : passV[ind - 1];
        }

        static Vector3 GetNext(int ind, Vector3[] passV)
        {
            return ind == passV.Length - 1 ? passV[0] : passV[ind + 1];
        }
        ...
        static Vector3 BezierPt(Vector3[] points, float t)
        {
            while (true)
            {
                ...
                for (int ii = 0; ii < lerps.Length; ii++) lerps[ii] = 
                    Vector3.Lerp(points[ii], points[ii + 1], t);
                points = lerps;
            }
        }
    }
