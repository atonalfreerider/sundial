...
public class Moon : MonoBehaviour {
    ...
    public const float lunarSynodic = 29.531f;
    ...
    public void NewMoon(float passMoonR, System.DateTime passDate) {
        ...
        moon.name = "Moon";
        moon.GetComponent<Renderer>().material = GameObject.FindGameObjectWithTag("SolarClock").GetComponent<SolarClock>().MoonMat;
    ...
    GameObject NewMoonSprockCont(float passMoonR, System.DateTime passDate) {
        ...
        for (int tt = 0; tt < 29; tt++) {
            alpha = -tt * step;
            retArr = Shapes.SprockTick(moonR, smallH, alpha, -(tt + 1) * step, sprockTh, baseAl, pointAl, pointCounter, 0f);
    ...
    public void MoveMoonSprockCont(System.DateTime passDate, bool dayChange, bool monthChange, bool forward) {
        moonSprockCont.transform.rotation = Quaternion.Euler(0f, moonSys.transform.rotation.eulerAngles.y + passDate.Hour * 360f / (24f * lunarSynodic), 0f);
        ...
        if (dayChange) {
            ...
            foreach (Text dL in dayList) {
                if (forward) {
                    ...
                    if (passDate.Month > 1) {
                        // if not January;
                        if ((monthChange && day > System.DateTime.DaysInMonth(passDate.Year, passDate.Month - 1)) || (!monthChange && day > System.DateTime.DaysInMonth(passDate.Year, passDate.Month)))
                        ...
                        if ((monthChange && day > System.DateTime.DaysInMonth(passDate.Year - 1, 12)) || (!monthChange && day > System.DateTime.DaysInMonth(passDate.Year, passDate.Month)))
                            day = 1;
                    }
                }
                else {
        ...
        monthSplitCont.transform.rotation = Quaternion.Euler(0f, moonSprock.transform.rotation.eulerAngles.y - daysUntilNextMonth * 360f / lunarSynodic, 0f);
