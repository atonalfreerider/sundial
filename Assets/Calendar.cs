...
public class Calendar {
    ...
    public GameObject yearCal;
    public bool vis = false;

    public List<MyEvent> yearQueue;
    public List<MyEvent> dayQueue;
    public static string[] daysofweek = new string[7] { "Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday" };
    public static string[] daysofweekAbr = new string[7] { "SUN", "MON", "TUE", "WED", "THU", "FRI", "SAT" };
    public static string[] monthofYrAbr = new string[12] { "JAN", "FEB", "MAR", "APR", "MAY", "JUN", "JUL", "AUG", "SEP", "OCT", "NOV", "DEC" };
    ...
    public void Init(float passYEAR, float passCR, float passER, float passSunSprockOffset) {
        ...
        while (calText.Contains("END:VEVENT")) {
            ...
            if (dtStart.Contains(";")) {
                ...
                yearQueue.Add(new MyEvent(evName, CreateDate(startDate[0], startDate[1], startDate[2], 0, 0), CreateDate(endDate[0], endDate[1], endDate[2], 0, 0), evColor));
            }
            else {
                // hourly event;
                ...
                dayQueue.Add(new MyEvent(evName, CreateDate(startDate[0], startDate[1], startDate[2], startDate[3] + TimeZone, startDate[4]), CreateDate(endDate[0], endDate[1], endDate[2], endDate[3] + TimeZone, endDate[4]), evColor));
            }
    ...
    public void DrawYearCalendar(System.DateTime passDate, Transform solarClock) {
        ...
        yearCal = new GameObject();
        yearCal.name = "YearCal";
    ...
    GameObject DrawEvent(MyEvent passEv, string type) {
        ...
        */
        ...
        if (type == "year")
            newRing.transform.rotation = Quaternion.Euler(new Vector3(0f, Orbits.getOrbitPos(passEv.end, YEAR, sunSprockOffset), 0f));
        else
            newRing.transform.rotation = Quaternion.Euler(new Vector3(0f, -360f * (passEv.end.Hour + passEv.end.Minute / 60f) / 24f, 0f));
