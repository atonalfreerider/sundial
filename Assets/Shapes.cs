...
public static class Shapes {
    ...
    public static object[] SprockTick(float R, float H, float alpha, float aDelt, float sprockTh, float baseAl, float pointAl, int counter, float spiralH) {
        List<Vector3> pointList = new List<Vector3>();
        List<int> indList = new List<int>();

        pointList.Add(new Vector3((R) * Mathf.Sin(alpha + baseAl), spiralH, (R) * Mathf.Cos(alpha + baseAl)));
        pointList.Add(new Vector3((R - sprockTh) * Mathf.Sin(alpha + baseAl), spiralH, (R - sprockTh) * Mathf.Cos(alpha + baseAl)));

        pointList.Add(new Vector3((R - H) * Mathf.Sin(alpha + pointAl), spiralH, (R - H) * Mathf.Cos(alpha + pointAl)));
        pointList.Add(new Vector3((R - H) * Mathf.Sin(alpha - pointAl), spiralH, (R - H) * Mathf.Cos(alpha - pointAl)));
    ...
    public static GameObject DrawLine(string type, Vector3 pt0, Vector3 pt1, Color passColor, float passLW, float passLD) {
        GameObject line;
    ...
    public static Vector3 VectArc(float R, float alpha, float prct, float offSet, float cX, float cY, float cZ) {
        return new Vector3(R * Mathf.Sin(alpha * prct * Mathf.PI / 180f + offSet * Mathf.PI / 180f) + cX, cY, R * Mathf.Cos(alpha * prct * Mathf.PI / 180f + offSet * Mathf.PI / 180f) + cZ);
    }

    public static GameObject DrawCirc(float R, float prct, Color passColor) {
